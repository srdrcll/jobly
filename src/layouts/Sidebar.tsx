    ...
    >
      ...
      <div>
        <div className="h-16 px-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-800/80">
          ...
          >
            <div className="p-2 rounded-xl bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 shadow-md group-hover:scale-105 transition-transform shrink-0">
              <Compass className="w-5 h-5" aria-hidden="true" />
            </div>
            {!isCollapsed && (
              <div className="flex flex-col">
                <span className="font-extrabold text-base tracking-tight text-foreground leading-none">
                  Kariyer <span className="text-slate-700 dark:text-slate-300">Pusulası</span>
                </span>
              ...
              {MAIN_NAV_ITEMS.map((item) => {
                ...
                return (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      cn(
                        ...
                        isActive
                          ? 'bg-slate-900 text-white dark:bg-slate-800 dark:text-slate-100 font-semibold shadow-sm'
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        <Icon className={cn('w-5 h-5 shrink-0 transition-transform group-hover:scale-110', isActive ? 'text-white dark:text-slate-100' : 'text-slate-400')} aria-hidden="true" />
                        {!isCollapsed && (
                          <span className="truncate flex-1">{item.title}</span>
                        )}
                        {!isCollapsed && getDynamicBadge(item.href) && (
                          <span className={cn('px-2 py-0.5 text-[10px] font-bold rounded-full border', isActive ? 'bg-slate-800 text-slate-100 border-slate-700 dark:bg-slate-700 dark:text-slate-200' : 'bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700')}>
