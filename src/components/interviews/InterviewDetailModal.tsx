...
export const InterviewDetailModal: React.FC<InterviewDetailModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'prep' | 'eval'>('overview');
  ...
  return (
    <Modal
      ...
      description={`${interview.stage || 'Mülakat'} Detay ve Hazırlık Modülü`}
      size="xl"
    >
      <div className="space-y-6 pt-2">
        ...
        <div className="flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-3 text-xs font-bold">
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-xl transition-all ${
              activeTab === 'overview'
                ...
                : 'text-slate-400 hover:text-foreground hover:bg-slate-800/40'
            }`}
          >
            <BookOpen className="w-3.5 h-3.5" /> Mülakat Bilgileri
          </button>
          ...
          >
            <Sparkles className="w-3.5 h-3.5" /> Hazırlık Araçları
          </button>
          ...
          >
            <Award className="w-3.5 h-3.5" /> Görüşme Değerlendirmesi
          </button>
        </div>
        ...
        {/* Tab 2: Preparation Tools */}
        {activeTab === 'prep' && <InterviewPrepSection interviewId={interview.id} />}

        {/* Tab 3: Evaluation */}
        {activeTab === 'eval' && <InterviewEvaluationSection interviewId={interview.id} />}
