...
export const EditCompanyModal: React.FC<EditCompanyModalProps> = ({ company, isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (company) {
      reset({
        ...
        notes: company.notes || '',
        status: (company.status as any) || 'Target',
  ...
  return (
    <Modal
      ...
      description={`${company?.name || ''} için detayları güncelleyin.`}
      size="lg"
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2 space-y-1.5">
            <label className="text-xs font-semibold text-foreground">Özel Notlar</label>
            <textarea
              rows={3}
              {...register('notes')}
        ...
        <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-foreground">Favori Şirketlerimde Tut</span>
          </div>
          <input
            type="checkbox"
            checked={isFavorite}
